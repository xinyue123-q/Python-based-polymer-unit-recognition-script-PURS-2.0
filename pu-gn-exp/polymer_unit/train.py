...
for epoch_idx in epoch_bar:
    ...
    for graphs, targets in dataloader_train:
        ...
        if experiment.session.losses.solubility > 0:
            ...
            loss_sol = criterion_mean(results.global_features.squeeze(),targets)
    ...
    for batch_idx, (graphs, targets) in enumerate(dataloader_val):
        ...
        if epoch_idx == experiment.session.epochs:
            #loss_sol_by_graph0 = F.mse_loss(
                #results.global_features.squeeze(), targets, reduction='none')
            loss_sol_by_graph = criterion(results.global_features.squeeze(),targets)
            pred = results.global_features.squeeze()
            pred = torch.argmax(pred,1,keepdim=True).squeeze()
            acc = accuracy_score(pred,targets)
            acc_list.append(acc)
