...
def get_pu_dist(file_name):
    ...
    ring_total_list0,total_neighbor_data0,total_inner_dist0,total_end_atom_pair=pu.get_pu(smi_list,name_list)
    ...
    total_neighbor_data2,total_inner_dist2,ring_total_list2 = pu.get_new_neighbor_data(total_neighbor_data,total_inner_dist,name_list,smi_list)
    matrix_list=pu.get_adj(ring_total_list2,total_neighbor_data2,name_list)
    pu_index = pu.get_pu_dict(total_neighbor_data2,ring_total_list2)
    pair_atom_dist = pu.get_pair_atom(total_neighbor_data2,total_inner_dist2)
    MACCS_dict = pu.get_MACCS(ring_total_list2,False)
    senders_list,receivers_list,edge_num_list = pu.edge_index(matrix_list)
    node_index, node_num_list = pu.get_node_index(total_neighbor_data2,ring_total_list2)
    edge_list,edge_num_list = pu.get_edge_feature(smi_list,name_list,matrix_list,pu_index,pair_atom_dist,total_inner_dist)
    ...
    for idx, name in enumerate(name_list):
        ...
        senders = senders_list[idx]
        receivers = receivers_list[idx] 
        graph=tg.Graph(
        num_nodes=num_nodes,
        num_edges=num_edges,
